// panel.js — Hightouch Events Debugger (MV3)
...
if (!window.__ht_isPreflight) {
  window.__ht_isPreflight = function (req) {
...
const isPreflight = window.__ht_isPreflight;

// ---------- state ----------
const state = {
  hosts: ['events.hightouch.com'],
  showNonEvents: false,
  showPreflight: false,
  counter: 0,
  entries: []
...
async function loadCustomExtractors() {
  try {
    ...
    // optional custom extractors; ignore failures
  }
}

function isEventRequest(url) {
  try {
    const u = new URL(url);
...
// For Hightouch we just keep basic URL info + selected headers; no db/table semantics
function extractRequestInfo(url, headersList) {
  const out = {};
  try {
    const u = new URL(url);
    out.host = u.host;
...
function loadPrefs() {
  chrome.storage.sync.get(
    ['htHosts', 'showNonEvents', 'htFilter', 'htRedact', 'showPreflight'],
    (res) => {
      if (Array.isArray(res.htHosts) && res.htHosts.length) state.hosts = res.htHosts;
      if (typeof res.showNonEvents === 'boolean') state.showNonEvents = res.showNonEvents;
      state.showPreflight = !!res.showPreflight;

      const hostsInput = $('hosts');
      if (hostsInput) hostsInput.value = state.hosts.join(', ');

      const nonEvents = $('showNonEvents');
      if (nonEvents) nonEvents.checked = state.showNonEvents;

      const pre = $('showPreflight');
      if (pre) pre.checked = state.showPreflight;

      const filt = $('filter');
      if (filt) filt.value = (res.htFilter || '');

      const red = $('redact');
      if (red) red.checked = !!res.htRedact;
    }
  );
}
...
function savePrefs() {
  ...
  const hosts = hostsInput
    ? hostsInput.value
        ...
        .filter(Boolean)
    : [];
  state.hosts = hosts.length ? hosts : ['events.hightouch.com'];
  state.showNonEvents = !!(nonEvents && nonEvents.checked);
  state.showPreflight = !!(pre && pre.checked);

  chrome.storage.sync.set({
    htHosts: state.hosts,
    showNonEvents: state.showNonEvents,
    showPreflight: state.showPreflight,
    htFilter: filt ? filt.value : '',
    htRedact: !!(red && red.checked)
  });
}
...
function attachNetworkListener() {
  chrome.devtools.network.onRequestFinished.addListener(async (req) => {
    ...
    const method = req.request && req.request.method;
    const isEvent = isEventRequest(url);
    if (!isEvent && !state.showNonEvents) return;
    ...
    const ctHdr = requestHeaders.find(
    ...
    const urlInfo = extractRequestInfo(url, requestHeaders);
    ...
    const finalize = async (bodyTextFromResp) => {
      ...
      const combined = mergeParsed(urlSummary, bodyParsed);
...
(function wireHandlers() {
  ...
  const nonEvents = $('showNonEvents');
  if (nonEvents) nonEvents.addEventListener('change', savePrefs);
  ...
  if (filterInput) {
    filterInput.addEventListener('input', () => {
      chrome.storage.sync.set({ htFilter: filterInput.value });
      renderTable();
    });
  }
  ...
  if (red) {
    red.addEventListener('change', () => {
      chrome.storage.sync.set({ htRedact: red.checked });
      renderTable();
    });
  }
  ...
  if (exportCsvBtn) {
    exportCsvBtn.addEventListener('click', () => {
      // CSV focused on Hightouch event fields
      const rows = [
        ['idx', 'time', 'method', 'status', 'url', 'type', 'userId', 'anonymousId', 'page_title', 'page_url']
      ];
      const qInput = $('filter');
      ...
      a.download = 'hightouch-events.csv';
      a.click();
  ...
  // Import / Export + redaction modal wiring
    ...
    importFileInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        if (obj.redactionRules) {
          allowed.redactionRules = Array.isArray(obj.redactionRules)
            ? obj.redactionRules
            : [];
        }
        if (obj.htHosts) allowed.htHosts = obj.htHosts;
        if (typeof obj.showNonEvents === 'boolean') {
          allowed.showNonEvents = obj.showNonEvents;
        }
        if (typeof obj.showPreflight === 'boolean') {
          allowed.showPreflight = obj.showPreflight;
        }
        if (typeof obj.htFilter === 'string') allowed.htFilter = obj.htFilter;
        if (typeof obj.htRedact === 'boolean') allowed.htRedact = obj.htRedact;
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', () => {
      chrome.storage.sync.get(
        ['customFields', 'redactionRules', 'htHosts', 'showNonEvents', 'showPreflight', 'htFilter', 'htRedact'],
        (res) => {
          const json = JSON.stringify(res, null, 2);
          const blob = new Blob([json], { type: 'application/json' });
          const a = document.createElement('a');
          a.href = URL.createObjectURL(blob);
          a.download = 'hightouch-events-debugger-settings.json';
          a.click();
          URL.revokeObjectURL(a.href);
        }
      );
